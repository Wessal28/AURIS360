...
function renderHeader(instance){
  ...
  (options.commands||[]).forEach(function(command){var button=header.querySelector('[data-layout-command="'+command.id+'"]');if(button)button.addEventListener('click',function(){if(typeof command.onClick==='function')command.onClick();});});
}
