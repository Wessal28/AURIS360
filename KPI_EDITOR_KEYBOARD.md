...
## Indicator add/remove follow-up (9 September 2026)

- Each indicator has a numbered visible heading, group and distinct accessible field/remove-button names. Numbers follow the current draft order; persisted indicator IDs are untouched.
- The existing Add/Remove CSP routes now call explicit editor actions. Add focuses the new name; remove focuses the next/previous available row, Add indicator when empty, or the editor title. An in-form status announces draft changes and is kept outside the row list so it cannot become saved indicator data. Loading/restoring rows does not steal focus or capture an incomplete draft. Restoration clears stale removal feedback.
- User actions check current company/account/permission/workflow context, visible controls and busy/written state. Removal changes only the draft. The existing save-time history check still refuses to delete an indicator with monthly records and offers Restore; this change does not bypass it or make any new backend requests.
- Added 24 regression tests. The initial 23-case baseline had 20 failures and 3 existing passing safeguards; the final extra case covers stale restoration feedback discovered in browser QA. Final full suite: 1,060 passed. Release readiness: 627 passed. No failures or skips; 180 offline routes and 22 migrations unchanged. The first full run found two old shared-handler cache-key assertions; those expectations were updated before the successful rerun.
- Browser skill used the real form/editor/CSP/upgrade hooks and styles with synthetic responses at 390x844 and 768x1024. Verified native Enter to add and Space to remove, next/previous focus, middle-row renumbering, history-removal refusal, Restore preserving the original ID and the other draft row, close/reopen draft recovery with title focus, and pending/partial-save Add/Remove disabling. Remove targets were 44x44px; no horizontal page overflow or console warnings/errors. External icon fonts are omitted by the fixture; physical Android, screen-reader certification and authenticated tenant journeys are not claimed.
- Shared handler cache keys, editor/CSS cache keys and offline manifest were updated together. No database migration, production record operation or production promotion is included. Keep PR #97 open and unmerged under the release boundary above.
