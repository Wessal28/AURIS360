...
function mount(host,rows,options){
  ...
  return root.AurisViewEngine.mount(host,projected,{moduleKey:'fleet-management',label:'Fleet Management',definition:definition(),context:function(){return assertSession(current);},filters:filters(options.filters),actions:[{key:'open',label:'Open vehicle'},{key:'check',label:'Monthly check',when:function(){return options.canEdit===true;}},{key:'edit',label:'Edit vehicle',when:function(){return options.canEdit===true;}}],onApplyFilters:function(value){assertSession(current);if(typeof options.onApplyFilters==='function')options.onApplyFilters(filters(value));},onAction:async function(action,row){
...
root.AurisFleetListWorkspace=Object.freeze({version:'1.0.0',mount:mount,project:project,definition:definition,filters:filters,checkState:checkState});
