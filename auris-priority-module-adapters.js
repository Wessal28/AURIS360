...
var modules={
  risk:{subtitle:'Assess hazards, controls and residual risk in one governed workspace',views:[{id:'dashboard',label:'Dashboard',group:'Overview'},{id:'mywork',label:'My Work',group:'My work'},{id:'register',label:'Risk Assessments',group:'Registers'},{id:'jsa',label:'JSA / JHA',group:'Registers'},{id:'hira',label:'HIRA',group:'Registers'}],primaryLabel:'New RA',onPrimary:function(){call('raShowNewPanel');},commands:[{id:'libraries',label:'Libraries',icon:'ti-books',onClick:function(){call('raShowLibrary');}}],onViewChange:function(id){['ra-new-panel','ra-form3view','ra-jsa-form','ra-library-view'].forEach(function(key){var el=button(key);if(el)el.style.display='none';});var list=button('ra-list-view');if(list)list.style.display='block';if(id==='dashboard'||id==='mywork'){call('raxSwitch',[id]);return;}call('raxSwitch',['register']);call('raListTab',[id==='register'?'ra':id,button('ra-ltab-'+(id==='register'?'ra':id))]);}},
...
function mount(key){
  ...
  return layout.mount(key,Object.assign({},config,{page:page,activeView:key==='risk'?'dashboard':(root.AurisModuleRegistry.get(key).layout||{}).defaultView,onRefresh:function(){var manifest=root.AurisModuleRegistry.get(key);call(manifest.loader);}}));
}
