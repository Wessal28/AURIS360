...
var modules={
  risk:{subtitle:'Assess hazards, controls and residual risk in one governed workspace',views:[{id:'register',label:'Risk Assessments',group:'Registers'},{id:'jsa',label:'JSA / JHA',group:'Registers'},{id:'hira',label:'HIRA',group:'Registers'}],primaryLabel:'New RA',onPrimary:function(){call('raShowNewPanel');},commands:[{id:'libraries',label:'Libraries',icon:'ti-books',onClick:function(){call('raShowLibrary');}}],onViewChange:function(id){['ra-new-panel','ra-form3view','ra-jsa-form','ra-library-view'].forEach(function(key){var el=button(key);if(el)el.style.display='none';});var list=button('ra-list-view');if(list)list.style.display='block';call('raxSwitch',['register']);call('raListTab',[id==='register'?'ra':id,button('ra-ltab-'+(id==='register'?'ra':id))]);}},
