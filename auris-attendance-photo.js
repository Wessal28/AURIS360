...
function mount(key,host,photo){
 ...
 ['Upload photo','Take photo'].forEach(function(label,i){var input=document.createElement('input');input.type='file';input.accept='image/jpeg,image/png,image/webp';input.hidden=true;if(i)input.setAttribute('capture','environment');input.addEventListener('change',function(){read(input.files[0]);input.value='';});var button=document.createElement('button');button.type='button';button.className='btn btn-sm';button.textContent=label;button.addEventListener('click',function(){input.click();});el.append(button,input);});
