...
function wsShowReadOnly(row,current,linkData,linked){
  ...
  var content=linked?'<section><h3>'+escH(linked.info.label)+'</h3>'+wsRecordFields(linked.row,Object.keys(linked.row).filter(function(key){return !['id','company_id','created_by','updated_by','attendance_photo'].includes(key)&&!/password|token/i.test(key);}).map(function(key){return [key,key.replace(/_/g,' ')];}))+'</section>':wsReadOnlyHtml(row);
  host.innerHTML='<section class="ws-record-window" role="dialog" aria-modal="true" aria-labelledby="ws-record-title"><header><div><p>Work Schedule · Read only</p><h2 id="ws-record-title">'+escH(title)+'</h2><p>'+escH(row.ref_number||'Draft')+'</p></div><button class="btn" data-ws-close>Close</button></header><nav aria-label="Work order actions">'+(!linked&&isMgr()?'<a class="btn" target="_blank" rel="noopener" href="'+escH(AurisWorkScheduleWorkspace.href(row.id,'edit',current.companyId))+'">Edit work order</a><a class="btn btn-primary" target="_blank" rel="noopener" href="'+escH(AurisWorkScheduleWorkspace.href(row.id,'manage',current.companyId))+'">Manage work & HSE checks</a>':'')+'</nav><div class="ws-record-content">'+content+(!linked?'<section><h3>Linked HSE records</h3>'+(linkData.warning?'<p role="alert">'+escH(linkData.warning)+'</p>':'')+(linkData.links.length?'<ul class="ws-record-links">'+linkData.links.map(function(link){return '<li><a target="_blank" rel="noopener" href="'+escH(AurisWorkScheduleWorkspace.href(row.id,'view',current.companyId,link))+'">'+escH(WS_RECORD_LINKS[link.kind].label)+' · '+escH(link.ref)+'</a></li>';}).join('')+'</ul>':'<p>No HSE records linked.</p>')+'</section>':'')+'</div></section>';
