...
function kpiXRenderScorecard(){
  ...
  kpiObjectives.forEach(function(obj){var objKpis=filtered.filter(function(k){return String(k.objective_id)===String(obj.id);});if(!objKpis.length)return;var objScores=objKpis.map(function(k){return k._kpiX.score;}).filter(function(x){return x!=null;}),objScore=objScores.length?kpiXRound(objScores.reduce(function(a,b){return a+b;},0)/objScores.length,0):0;html+='<tr class="objective"><td colspan="8"><span class="kpi-x-hierarchy-code">'+kpiXEsc(obj.code||'')+'</span>'+kpiXEsc(obj.name)+' <span class="kpi-x-objective-score">'+objScore+'%</span></td></tr>';
    objKpis.forEach(function(k){var status=k._computed_status,inds=kpiIndicators.filter(function(ind){return String(ind.kpi_id)===String(k.id);});html+='<tr class="kpi-row" data-auris-module-onclick="d0003" data-auris-module-args="'+encodeURIComponent(JSON.stringify([kpiXEsc(k.id)]))+'"><td><div class="kpi-x-hierarchy kpi"><span class="kpi-x-hierarchy-code">'+kpiXEsc(k.code||'')+'</span><strong>'+kpiXEsc(k.name)+'</strong></div></td><td><span class="kpi-x-type">KPI</span><small>'+kpiXEsc(k.frequency||'Monthly')+'</small></td><td>—</td><td>'+(k._kpiX.score==null?'—':'<strong>'+k._kpiX.score+'%</strong><small>Achievement</small>')+'</td><td>—</td><td>—</td><td><span class="kpi-x-owner">'+kpiXInitials(kpiXOwner(k))+'</span>'+kpiXEsc(kpiXOwner(k))+'</td><td>'+kpiXStatusHtml(status)+'</td></tr>';
      inds.forEach(function(ind){var compilationMonth=kpiXCompilationMonth(),dueMonth=kpiXDueMonth(k,compilationMonth),snap=dueMonth?kpiXIndicatorSnapshot(ind,dueMonth):{status:'not_due',actual:null,variance:null},latest=kpiXLatestRow(ind.id,dueMonth||compilationMonth),ytd=latest&&latest.ytd!=null?latest.ytd:(latest&&latest.actual),classification=ind.indicator_type||ind.classification||ind.type||'Indicator';html+='<tr class="kpi-x-indicator-row" data-auris-module-onclick="d0003" data-auris-module-args="'+encodeURIComponent(JSON.stringify([kpiXEsc(k.id)]))+'"><td><div class="kpi-x-hierarchy indicator"><span class="kpi-x-hierarchy-code">'+kpiXEsc(ind.code||'•')+'</span><span>'+kpiXEsc(ind.name)+'</span></div></td><td><span class="kpi-x-type">'+kpiXEsc(classification)+'</span></td><td><strong>'+kpiXEsc(kpiXTargetText(ind))+'</strong></td><td>'+(snap.actual==null?kpiXEsc(kpiXStatusLabel(snap.status)):'<strong>'+kpiXEsc(kpiXRound(snap.actual,2))+'</strong><small>YTD '+kpiXEsc(ytd==null?'—':kpiXRound(ytd,2))+'</small>')+'</td><td>'+kpiXEsc(kpiXVarianceText(ind,snap))+'</td><td>'+kpiXSpark(ind)+'</td><td>'+kpiXEsc(kpiXOwner(k))+'</td><td>'+kpiXStatusHtml(snap.status)+'</td></tr>';});
    });
  });
