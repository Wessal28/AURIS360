...
function kpiXRenderMonthly(){
  ...
  kpiObjectives.forEach(function(obj){var rows=kpiXFilteredKpis().filter(function(k){return String(k.objective_id)===String(obj.id);});if(!rows.length)return;html+='<tr><td colspan="'+(11+groups.length)+'" style="background:#eff8f5!important;color:#087252;font-weight:800">'+kpiXEsc((obj.code||'')+'. '+obj.name)+'</td></tr>';rows.forEach(function(k){var inds=kpiIndicators.filter(function(ind){return String(ind.kpi_id)===String(k.id);});inds.forEach(function(ind,index){var current=kpiXIndicatorSnapshot(ind,monthNow),direction=kpiXDirection(ind);html+='<tr data-kpi-id="'+kpiXEsc(k.id)+'"><td class="kpi-x-sticky-1">'+(index===0?kpiXEsc(k.code||''):'')+'</td><td class="kpi-x-sticky-2">'+(index===0?'<strong>'+kpiXEsc(k.name)+'</strong>':'')+'</td><td class="kpi-x-sticky-3">'+kpiXEsc(ind.name)+'</td><td>'+kpiXEsc(ind.unit||'count')+'</td><td>'+direction.arrow+'</td><td><strong>'+kpiXEsc(kpiXTargetText(ind))+'</strong></td>';
...
function kpiXRefreshHeader(){var label=document.getElementById('kpi-co-label');if(label)label.textContent=(typeof co!=='undefined'&&co&&co.name)||(typeof companyName==='function'?companyName():'Company performance');}
