...
function kpiXEvaluate(ind,actual,previous){
  ...
  return {status:ok?'on_track':near?'at_risk':'off_track',score:kpiXRound(Math.min(100,Math.max(0,score)),0),variance:variance};
}
function kpiXIndicatorSnapshot(ind,month){
  var row=((typeof kpiMonthlyData!=='undefined'&&kpiMonthlyData[ind.id])||{})[month];
  if(!row)return {row:null,status:month>kpiXReportingMonth()?'not_due':'data_missing',score:null,actual:null,variance:null};
...
function kpiXKpiSnapshot(k,month){
  var indicators=(typeof kpiIndicators!=='undefined'?kpiIndicators:[]).filter(function(ind){return ind.kpi_id===k.id;});
  ...
  var statuses=snapshots.map(function(x){return x.status;});
  var status=statuses.indexOf('off_track')>=0?'off_track':statuses.indexOf('data_missing')>=0?'data_missing':statuses.indexOf('at_risk')>=0?'at_risk':statuses.every(function(x){return x==='not_due';})?'not_due':statuses.every(function(x){return x==='not_started';})?'not_started':'on_track';
  var scores=snapshots.map(function(x){return x.score;}).filter(function(x){return x!=null;});
  return {status:status,score:scores.length?kpiXRound(scores.reduce(function(a,b){return a+b;},0)/scores.length,0):null,indicators:snapshots};
}
function kpiXCompute(month){
  month=month||kpiXReportingMonth()||1;
...
function kpiXStatusLabel(status){return {on_track:'On Track',at_risk:'At Risk',off_track:'Off Track',data_missing:'Data Missing',not_started:'Not Started',not_due:'Not Due'}[status]||'Not Started';}
function kpiXStatusHtml(status){
  var icon={on_track:'ti-circle-check',at_risk:'ti-alert-triangle',off_track:'ti-circle-x',data_missing:'ti-help-circle',not_started:'ti-clock',not_due:'ti-calendar-time'}[status]||'ti-clock';
...
function kpiXFilteredKpis(){
  ...
  return rows.filter(function(k){
    ...
    if(kpiXState.objective!=='all'&&k.objective_id!==kpiXState.objective)return false;
...
function kpiXTrendSvg(){
  ...
  for(var month=1;month<=12;month++){
    var scores=[];kpiIndicators.forEach(function(ind){var snap=kpiXIndicatorSnapshot(ind,month);if(snap.score!=null)scores.push(snap.score);});
...
function kpiXRenderScorecard(){
  ...
  kpiXCompute();var filtered=kpiXFilteredKpis(),html='<div class="kpi-x-table-wrap"><table class="kpi-x-table"><thead><tr><th>Code</th><th>KPI / Indicator</th><th>Direction</th><th>Unit</th><th>Target '+kpiXSelectedYear()+'</th><th>Current Actual</th><th>Variance</th><th>Trend</th><th>KPI Owner</th><th>Frequency</th><th>Status</th><th style="text-align:right">Actions</th></tr></thead><tbody>';
  kpiObjectives.forEach(function(obj){var objKpis=filtered.filter(function(k){return k.objective_id===obj.id;});if(!objKpis.length)return;var objScores=objKpis.map(function(k){return k._kpiX.score;}).filter(function(x){return x!=null;}),objScore=objScores.length?kpiXRound(objScores.reduce(function(a,b){return a+b;},0)/objScores.length,0):0;html+='<tr class="objective"><td colspan="12">'+kpiXEsc((obj.code||'')+'. '+obj.name)+' <span style="float:right">'+objScore+'%</span></td></tr>';
    objKpis.forEach(function(k){var status=k._computed_status,inds=kpiIndicators.filter(function(ind){return ind.kpi_id===k.id;});html+='<tr class="kpi-row" onclick="kpiXOpenDrawer(\''+kpiXEsc(k.id)+'\')"><td>'+kpiXEsc(k.code||'')+'</td><td>'+kpiXEsc(k.name)+'</td><td>—</td><td>—</td><td>—</td><td>'+(k._kpiX.score==null?'—':k._kpiX.score+'%')+'</td><td>—</td><td>—</td><td><span class="kpi-x-owner">'+kpiXInitials(kpiXOwner(k))+'</span>'+kpiXEsc(kpiXOwner(k))+'</td><td>'+kpiXEsc(k.frequency||'Monthly')+'</td><td>'+kpiXStatusHtml(status)+'</td><td><div class="kpi-x-row-actions"><button class="kpi-x-icon-btn" title="Details" onclick="event.stopPropagation();kpiXOpenDrawer(\''+kpiXEsc(k.id)+'\')"><i class="ti ti-eye"></i></button>'+(['at_risk','off_track'].indexOf(status)>=0?'<button class="kpi-x-icon-btn action" title="Create corrective action" onclick="event.stopPropagation();kpiXCreateAction(\''+kpiXEsc(k.id)+'\')"><i class="ti ti-plus"></i></button>':'')+'</div></td></tr>';
      inds.forEach(function(ind){var dueMonth=kpiXDueMonth(k,kpiXReportingMonth()),snap=dueMonth?kpiXIndicatorSnapshot(ind,dueMonth):{status:'not_due',actual:null,variance:null},direction=kpiXDirection(ind);html+='<tr onclick="kpiXOpenDrawer(\''+kpiXEsc(k.id)+'\')"><td style="padding-left:20px!important;color:#64748b">'+kpiXEsc(ind.code||'•')+'</td><td style="padding-left:24px!important">'+kpiXEsc(ind.name)+'</td><td><span class="kpi-x-direction">'+direction.arrow+' '+direction.label+'</span></td><td>'+kpiXEsc(ind.unit||'count')+'</td><td><strong>'+kpiXEsc(kpiXTargetText(ind))+'</strong></td><td>'+(snap.actual==null?'—':kpiXEsc(kpiXRound(snap.actual,2)))+'</td><td>'+kpiXEsc(kpiXVarianceText(ind,snap))+'</td><td>'+kpiXSpark(ind)+'</td><td>'+kpiXEsc(kpiXOwner(k))+'</td><td>'+kpiXEsc(k.frequency||'Monthly')+'</td><td>'+kpiXStatusHtml(snap.status)+'</td><td></td></tr>';});
    });
  });
...
function kpiXRenderMonthly(){
  ...
  var html='',monthNow=kpiXReportingMonth();
  kpiObjectives.forEach(function(obj){var rows=kpiXFilteredKpis().filter(function(k){return k.objective_id===obj.id;});if(!rows.length)return;html+='<tr><td colspan="'+(11+groups.length)+'" style="background:#eff8f5!important;color:#087252;font-weight:800">'+kpiXEsc((obj.code||'')+'. '+obj.name)+'</td></tr>';rows.forEach(function(k){var inds=kpiIndicators.filter(function(ind){return ind.kpi_id===k.id;});inds.forEach(function(ind,index){var current=kpiXIndicatorSnapshot(ind,monthNow),direction=kpiXDirection(ind);html+='<tr data-kpi-id="'+kpiXEsc(k.id)+'"><td class="kpi-x-sticky-1">'+(index===0?kpiXEsc(k.code||''):'')+'</td><td class="kpi-x-sticky-2">'+(index===0?'<strong>'+kpiXEsc(k.name)+'</strong>':'')+'</td><td class="kpi-x-sticky-3">'+kpiXEsc(ind.name)+'</td><td>'+kpiXEsc(ind.unit||'count')+'</td><td>'+direction.arrow+'</td><td><strong>'+kpiXEsc(kpiXTargetText(ind))+'</strong></td>';
      groups.forEach(function(months){var lastMonth=months[months.length-1],future=months[0]>monthNow,applicable=period!=='monthly'||kpiXIsDue(k,lastMonth),row=applicable?(period==='monthly'?((kpiMonthlyData[ind.id]||{})[lastMonth]):kpiXAggregateRows(ind,months)):null,evaluation=!applicable?{status:'not_due'}:row?kpiXEvaluate(ind,row.actual,(kpiXPreviousRow(ind.id,months[0])||{}).actual):{status:future?'not_due':'data_missing'};var text=!applicable?'N/A':future?'Not Due':row?kpiXRound(row.actual,2):'No Data';html+='<td class="kpi-x-cell '+evaluation.status+'" '+(applicable&&!future&&kpiCanEdit()?'onclick="kpiOpenEntry(\''+kpiXEsc(ind.id)+'\',\''+kpiXEsc(k.id)+'\','+lastMonth+')"':'')+'>'+kpiXEsc(text)+'</td>';});
      var latest=kpiXLatestRow(ind.id,monthNow),ytd=latest&&latest.ytd!=null?latest.ytd:(latest&&latest.actual);html+='<td><strong>'+(ytd==null?'—':kpiXEsc(kpiXRound(ytd,2)))+'</strong></td><td>'+kpiXEsc(kpiXVarianceText(ind,current))+'</td><td>'+kpiXSpark(ind)+'</td><td>'+kpiXStatusHtml(current.status)+'</td><td><button class="kpi-x-icon-btn" onclick="kpiXOpenDrawer(\''+kpiXEsc(k.id)+'\')"><i class="ti ti-dots"></i></button></td></tr>';});});});
...
function kpiXRenderCycleBanner(){
  var host=document.getElementById('kpi-x-cycle');if(!host)return;var month=kpiXReportingMonth()||1,dueIndicators=kpiIndicators.filter(function(ind){var k=kpiKPIs.find(function(item){return item.id===ind.kpi_id;});return kpiXIsDue(k,month);}),total=dueIndicators.length,complete=dueIndicators.filter(function(ind){return !!((kpiMonthlyData[ind.id]||{})[month]);}).length,pct=total?Math.round(complete/total*100):100,missing=Math.max(0,total-complete);
  host.innerHTML='<i class="ti ti-info-circle"></i><strong>'+KPI_X_MONTHS[month-1]+' '+kpiXSelectedYear()+' reporting cycle</strong><span>'+complete+' of '+total+' indicators completed</span><span>• '+missing+' missing</span><span class="kpi-x-cycle-progress"><i style="width:'+pct+'%"></i></span><strong>'+pct+'%</strong><button class="kpi-x-btn" style="height:29px;margin-left:auto" onclick="kpiXReviewMissing()">Review missing</button>';
}
...
function kpiXRenderAll(){kpiXRefreshFilters();kpiXRenderDashboard();kpiXRenderScorecard();kpiXRenderMonthly();kpiXRenderActions();kpiXRenderReports();kpiXRenderConfig();}
function kpiXSwitchTab(tab){
  kpiXState.tab=tab;document.querySelectorAll('#page-kpi .kpi-x-view').forEach(function(view){view.hidden=view.getAttribute('data-kpi-view')!==tab;});document.querySelectorAll('#page-kpi .kpi-tab').forEach(function(btn){btn.classList.toggle('active',btn.getAttribute('data-tab')===tab);});
...
function kpiXInstall(){
  ...
  header.className='kpi-x-header';header.innerHTML='<div><div class="kpi-x-title">Objectives & KPIs</div><div class="kpi-x-company" id="kpi-co-label"></div></div><div class="kpi-x-actions"><select id="year-sel" onchange="kpiLoadAll()" style="height:38px;border:1px solid #d9e1ea;border-radius:9px;padding:0 10px;background:#fff"><option>2024</option><option>2025</option><option>2026</option><option>2027</option><option>2028</option></select><button class="kpi-x-btn primary" onclick="openKpiAddModal()"><i class="ti ti-plus"></i>New KPI</button><button class="kpi-x-btn" onclick="kpiXSubmitMonth()"><i class="ti ti-circle-check"></i>Submit Month</button><button class="kpi-x-btn" onclick="kpiXExportCsv()"><i class="ti ti-download"></i>Export Report</button></div>';document.getElementById('year-sel').value=selectedYear;
  var filter=document.createElement('div');filter.className='kpi-x-filters';filter.innerHTML='<div class="kpi-x-field"><label>Search</label><input id="kpi-x-search" placeholder="Search KPIs..." oninput="kpiXSetSearch(this.value)"></div><div class="kpi-x-field"><label>Objective</label><select id="kpi-x-objective" onchange="kpiXSetObjective(this.value)"><option value="all">All Objectives</option></select></div><div class="kpi-x-field"><label>KPI Owner</label><select id="kpi-x-owner" onchange="kpiXSetOwner(this.value)"><option value="all">All KPI Owners</option></select></div><div class="kpi-x-field"><label>Status</label><select id="kpi-x-status" onchange="kpiXFilterStatus(this.value)"><option value="all">All Statuses</option><option value="on_track">On Track</option><option value="at_risk">At Risk</option><option value="off_track">Off Track</option><option value="data_missing">Data Missing</option></select></div><div class="kpi-x-field"><label>Frequency</label><select id="kpi-x-frequency" onchange="kpiXSetFrequency(this.value)"><option value="all">All Frequencies</option><option value="monthly">Monthly</option><option value="quarterly">Quarterly</option><option value="annual">Annual</option></select></div><button class="kpi-x-btn" onclick="kpiXResetFilters()"><i class="ti ti-refresh"></i>Reset</button>';header.insertAdjacentElement('afterend',filter);
  tabs.className='kpi-x-tabs';tabs.innerHTML=[['dashboard','Dashboard'],['scorecard','KPI Scorecard'],['monthly','Monthly Tracking'],['actions','Action Plans'],['reports','Reports'],['configuration','Configuration']].map(function(item){return '<button class="kpi-tab" data-tab="'+item[0]+'" onclick="kpiXSwitchTab(\''+item[0]+'\')">'+item[1]+'</button>';}).join('');
  ...
  kpiXEnhanceEntryModal();kpiXEnhanceKpiModal();kpiXSwitchTab('dashboard');
}
...
function kpiXInstallHooks(){
  if(typeof window.kpiLoadAll==='function'){kpiXLegacy.loadAll=window.kpiLoadAll;window.kpiLoadAll=async function(){await kpiXLegacy.loadAll.apply(this,arguments);kpiXCompute();kpiXRenderAll();};}
...
window.kpiXSwitchTab=kpiXSwitchTab;window.kpiXFilterStatus=kpiXFilterStatus;window.kpiXFilterObjective=kpiXFilterObjective;window.kpiXSetObjective=kpiXSetObjective;window.kpiXSetOwner=kpiXSetOwner;window.kpiXSetSearch=kpiXSetSearch;window.kpiXResetFilters=kpiXResetFilters;window.kpiXSetPeriod=kpiXSetPeriod;window.kpiXReviewExceptions=kpiXReviewExceptions;window.kpiXReviewMissing=kpiXReviewMissing;window.kpiXSubmitMonth=kpiXSubmitMonth;window.kpiXExportCsv=kpiXExportCsv;window.kpiXOpenDrawer=kpiXOpenDrawer;window.kpiXCreateAction=kpiXCreateAction;window.kpiXGoMonthly=kpiXGoMonthly;
...
function kpiXBoot(){
  ...
  var label=document.getElementById('kpi-co-label');if(label)label.textContent=(typeof co!=='undefined'&&co&&co.name)||(typeof companyName==='function'?companyName():'Company performance');
}
