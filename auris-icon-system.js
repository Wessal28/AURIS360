...
var sidebarHandlerKeys={h0027:'dashboard',h0028:'executive',h0029:'ai-insights',h0030:'kpi',h0031:'workschedule',h0032:'events',h0033:'observation',h0034:'inspection',h0035:'risk',h0036:'tools',h0037:'fleet',h0038:'atex',h0039:'sitemap',h0040:'permit',h0041:'contractor',h0042:'emergency',h0043:'ohealth',h0044:'ppe',h0045:'fire',h0046:'chemical',h0047:'esg',h0048:'noise',h0049:'meetings',h0050:'training',h0051:'actions',h0052:'legal',h0053:'sop',h0054:'swms',h0055:'documents',h0056:'people',h0057:'users',h0058:'admin',h0059:'integrations',h0060:'approvals',h0061:'audit',h0062:'settings'};
var queued=false;
function navKey(el){if(el.dataset.appKey)return el.dataset.appKey;if(el.dataset.navKey)return el.dataset.navKey;if(el.dataset.page)return el.dataset.page;var handler=el.getAttribute('data-auris-onclick')||'';if(sidebarHandlerKeys[handler])return sidebarHandlerKeys[handler];var click=el.getAttribute('onclick')||'';var m=click.match(/(?:showPage|modulesMenuNavigate|mobileNavTo)\('([^']+)'/);if(m)return m[1];var id=el.id||'';if(id.indexOf('mob-btn-')===0)return id.slice(8);if(id.indexOf('nav-')===0){var alias={ai:'ai-insights',mtg:'meetings',docs:'documents'};return alias[id.slice(4)]||id.slice(4);}return '';}
function applyModules(){document.querySelectorAll('.sidebar .nav-item,.modules-menu-item,.auris-app-card[data-app-key],.mob-nav-btn,.mob-module-btn,.mob-search-result').forEach(function(el){var key=navKey(el);if(!key)return;el.dataset.navKey=key;var icon=el.querySelector(el.matches('.auris-app-card')?'.auris-app-card-icon > i':'i');if(icon)icon.classList.toggle('auris-module-icon',moduleKeys.has(key));});}
