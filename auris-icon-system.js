...
var queued=false;
function navKey(el){if(el.dataset.navKey)return el.dataset.navKey;if(el.dataset.page)return el.dataset.page;var click=el.getAttribute('onclick')||'';var m=click.match(/(?:showPage|modulesMenuNavigate|mobileNavTo)\('([^']+)'/);if(m)return m[1];var id=el.id||'';if(id.indexOf('mob-btn-')===0)return id.slice(8);return '';}
