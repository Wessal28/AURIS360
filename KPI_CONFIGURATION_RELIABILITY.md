...
| Exclude open current period | Zero-month compiled latest/YTD lookup corrected in the reporting cut-off phase. CSV now uses the same compilation cut-off; full-year raw charts are accurately labelled Recorded trend rather than six-month performance. See `KPI_REPORTING_PERIOD_CUTOFF.md` and `KPI_REPORTING_CONSISTENCY.md`; changing trend policy or scoring meaning remains separate. |
