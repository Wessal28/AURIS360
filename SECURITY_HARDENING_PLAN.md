...
This is a targeted high-risk sink remediation. CSP phase 1 is recorded in `CSP_HARDENING_PHASE_1_2026-08-14.md`: both inline script blocks are external assets, generated SOP print scripts are removed and `script-src-elem` is enforced. CSP phases 2A through 2C removed every inline handler from the core page and core script. Phase 2D migrated the first ten loaded module bundles, leaving 336 handlers in the remaining bundles; inline styles also remain.

## Remaining gates

1. Replace the remaining 336 event-handler attributes in loaded module bundles and continue reviewing module-specific rich-content renderers.
