...
function malformedAnalysis(error){return error instanceof SyntaxError||/JSON|double-quoted property|Unexpected token|Unexpected end/i.test(String(error&&error.message||error||''));}
function analysisFailure(error){var message=String(error&&error.message||error||'PDF analysis failed.');if(malformedAnalysis(error))return {message:'The AI analysis response was incomplete or incorrectly formatted.',hint:'Please select Analyse PDF again. The system will retry safely and no records have been created.'};if(/output limit|incomplete response/i.test(message))return {message:message,hint:'Try Analyse PDF again. If the document is very large, split it into two smaller searchable PDFs.'};if(/No readable text|scanned image|OCR/i.test(message))return {message:message,hint:'If the PDF is scanned, run OCR or export it as a searchable PDF and try again.'};return {message:message,hint:'Please try again. No objectives or KPIs are created until the review is confirmed.'};}
async function analyse(){
  ...
  try{
    ...
    var aiOptions={max_tokens:12000,response_schema:{name:'kpi_document_import',schema:KPI_PDF_SCHEMA}},raw=await callAI([{role:'user',content:parts}],'You are a conservative HSE performance-management document extraction engine. Preserve source meaning, never manufacture facts, and return valid JSON only.',aiOptions);
    try{kpiPdfState.analysis=cleanAnalysis(raw);}catch(parseError){if(!malformedAnalysis(parseError))throw parseError;setBusy(true,'Retrying analysis...');raw=await callAI([{role:'user',content:parts}],'You are a conservative HSE performance-management document extraction engine. A previous response was malformed. Preserve every explicitly stated objective, KPI and indicator, never manufacture facts, and return one complete valid JSON object only.',aiOptions);kpiPdfState.analysis=cleanAnalysis(raw);}renderReview(extracted.characters||extracted.text.length);
  ...
  }catch(error){var failure=analysisFailure(error);result.innerHTML='<div class="kpi-pdf-error"><i class="ti ti-alert-triangle"></i><div><strong>PDF could not be analysed</strong><p>'+esc(failure.message)+'</p><span>'+esc(failure.hint)+'</span></div></div>';message('PDF analysis failed.',false);}finally{setBusy(false);}
}
