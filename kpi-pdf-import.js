...
async function createRecords(){
  if(kpiPdfState.busy)return;var draft=readDraft(),counts={objectives:draft.length,kpis:0,indicators:0};draft.forEach(function(o){counts.kpis+=o.kpis.length;o.kpis.forEach(function(k){counts.indicators+=k.indicators.length;});});if(!counts.objectives||!counts.kpis){message('Keep at least one named objective and KPI before creating records.',false);return;}if(!(await confirmCreate(counts)))return;
  var button=document.getElementById('kpi-pdf-create');kpiPdfState.busy=true;button.disabled=true;button.innerHTML='<i class="ti ti-loader-2 kpi-pdf-spin"></i>Creating…';var result={objectives:0,reused:0,kpis:0,indicators:0,duplicates:0,failures:[]},cid=companyId(),year=parseInt(document.getElementById('kpi-pdf-year').value,10)||selectedYear();
  try{
