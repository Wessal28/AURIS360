...
test('deployment loads an accessible responsive command centre before core',()=>{
  ...
  assert.ok(html.indexOf('auris-command-centre.js?v=20260911-trigger-fix-1')<html.indexOf('auris-core.js?v=20260903-29'));
  ...
  assert.match(css,/@media\(max-width:600px\)/);assert.match(css,/focus-visible/);assert.match(runtime,/modular-foundation-29/);
});
