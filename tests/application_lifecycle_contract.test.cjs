...
test('operations UI and deployment expose version, health and release identity',()=>{
  ...
  assert.match(html,/modular-foundation-13/);assert.match(html,/settings-application-lifecycle-card/);assert.ok(html.indexOf('auris-application-lifecycle.js?v=20260901-13')<html.indexOf('auris-core.js?v=20260901-11'));
  ...
  for(const marker of ["build: '2026-09-01-modular-foundation-13'","platformVersion: '2.0.0'","moduleRegistryVersion: '2.1.0'"])assert.match(api,new RegExp(marker.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));
