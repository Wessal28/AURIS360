...
test('KPI drawer exposes assigned review, approval, rejection, revision and locking controls',()=>{
  ...
  assert.ok(html.indexOf('kpi-module-upgrade.js?v=20260908-clear-entry-1')<html.indexOf('kpi-workflow.js?v=20260904-locked-monthly-1'));
});
...
test('current build markers and cache keys are published',()=>{
  ...
  assert.match(html,/kpi-module-upgrade\.js\?v=20260908-clear-entry-1/);
