...
test('KPI drawer exposes assigned review, approval, rejection, revision and locking controls',()=>{
  ...
  assert.match(feature,/version:'1\.1\.2'/);
  ...
  assert.match(css,/\.kpi-x-drawer-actions \{[^}]*display:grid[^}]*grid-template-columns:repeat\(2,minmax\(0,1fr\)\)/);
  ...
  assert.ok(html.indexOf('kpi-module-upgrade.js?v=20260909-config-editor-recovery-1')<html.indexOf('kpi-workflow.js?v=20260904-locked-monthly-1'));
});
...
test('current build markers and cache keys are published',()=>{
  ...
  assert.match(html,/kpi-module-upgrade\.js\?v=20260909-config-editor-recovery-1/);
  assert.match(html,/kpi-module-upgrade\.css\?v=20260909-config-editor-recovery-1/);
  assert.match(html,/kpi-workflow\.js\?v=20260904-locked-monthly-1/);
