...
function harness({realLoader=true,existing=false}={}){
  ...
  controls.forEach(el=>{el.matches=selector=>selector==='input,textarea'?['entry-actual','entry-ytd','entry-comment','entry-root-cause','entry-evidence'].some(id=>nodes[id]===el):selector.includes('h0143')&&el===cancel;});
