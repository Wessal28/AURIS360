...
| Mobile/offline | Run `npm run test:mobile-offline` and `npm run test:offline-diagnostic`. In Settings, run **Mobile/offline synchronization preflight** and retain its result; it safely checks browser-session storage, photo retention, duplicate protection and company isolation with synthetic drafts and no API writes. Then, on protected desktop, tablet and installed mobile-PWA sessions: open an authenticated record link; create one Incident and one BBS draft while offline; reconnect under the same company and verify each syncs once with its photos; confirm a draft created under another company is retained but cannot sync until that company is selected. Verify unsupported modules clearly require connectivity. Capture screenshots and resulting record IDs. The safe preflight does not replace this live evidence. |
| Rollback | Run `npm run test:controlled-rollback`, `npm run test:rollback-rehearsal` and `npm run test:rollout-evidence`. In Settings, run **Controlled rollback rehearsal**; it safely exercises the shared Pilot → Paused → Disabled visibility policy with synthetic session-only evidence and no API writes. Then apply the rerunnable `controlled_rollout_upgrade.sql` to a protected test tenant. In a quiet test window, run `core_control_rollout_evidence_snapshot.sql` immediately before and after setting one authorised cohort Pilot, then Paused and Disabled. Confirm its modules disappear immediately, an open gated module returns to Dashboard, other cohorts remain available, activation/gate evidence and transition history remain intact, all four business-table counts are unchanged, and the selected company's `snapshot_fingerprint` is identical. Restore Pilot only after its required gates pass. Never delete a cohort policy to roll back. The safe rehearsal does not replace the protected-tenant exercise. |
