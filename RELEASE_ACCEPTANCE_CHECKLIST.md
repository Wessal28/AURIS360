...
| Rollback | Run `npm run test:controlled-rollback` and `npm run test:rollback-rehearsal`. In Settings, run **Controlled rollback rehearsal**; it safely exercises the shared Pilot → Paused → Disabled visibility policy with synthetic session-only evidence and no API writes. Then apply the rerunnable `controlled_rollout_upgrade.sql` to a protected test tenant. Record business-table counts, set one authorised cohort Pilot, then Paused and Disabled; confirm its modules disappear immediately, an open gated module returns to Dashboard, other cohorts remain available, activation/gate evidence and transition history remain intact, and all business-table counts are unchanged. Restore Pilot only after its required gates pass. Never delete a cohort policy to roll back. The safe rehearsal does not replace the protected-tenant exercise. |
