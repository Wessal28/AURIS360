...
function mount(host,rows,options){
  ...
  return root.AurisViewEngine.mount(host,projected,{moduleKey:'tools-equipment',label:'Tools & Equipment',definition:definition(),context:function(){return assertSession(current);},filters:filters(options.filters),actions:[{key:'open',label:'View equipment',href:typeof options.recordHref==='function'?function(row){return options.recordHref(row.id,'view');}:undefined},{key:'inspect',label:'Start inspection'},{key:'edit',label:'Edit equipment',href:typeof options.recordHref==='function'?function(row){return options.recordHref(row.id,'edit');}:undefined,when:function(){return options.canEdit===true;}}],onApplyFilters:function(value){assertSession(current);if(typeof options.onApplyFilters==='function')options.onApplyFilters(filters(value));},onAction:async function(key,row){
...
root.AurisToolsListWorkspace=Object.freeze({version:'1.1.0',session:session,assertSession:assertSession,mount:mount,project:project,definition:definition,filters:filters,inspectionFor:inspectionFor});
