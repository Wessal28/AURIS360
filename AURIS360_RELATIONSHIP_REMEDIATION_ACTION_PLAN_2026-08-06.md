...
- **Owner:** Product/Release manager and environment tester
- **Status (2026-08-14):** In progress — the live AURIS360 Supabase project passed the complete read-only Core Control gate verification: 4/4 rollout relations, transition trigger installed, RLS enabled on 3/3 protected tables, no unsafe enabled cohort policy, no open rollout-health finding and 118 Core Control records with no missing company relationship. No cohort policy is saved, so all tenants remain in compatibility mode. Supabase Advisor reports no issues. Authenticated Dashboard and all four Core Control modules rendered live data without schema/setup errors; 1440, 1024 and 390 px smoke checks had no page-level horizontal overflow. Session-scoped administrator tools safely exercise optional dependency resilience, mobile/offline storage and Core Control Pilot → Paused → Disabled policy without API writes or production access changes. The production offline queue has a single-flight reconnect guard. A read-only protected-tenant evidence snapshot now reports gate flags, health findings, transition history, all four Core Control table counts and a comparison fingerprint before and after an authorised rollback exercise. Captured live resilience evidence, real-device Incident/BBS offline synchronization and the authorised protected-tenant Pilot-to-Paused-to-Disabled exercise remain pending; no cohort was enabled and no production record was changed.
