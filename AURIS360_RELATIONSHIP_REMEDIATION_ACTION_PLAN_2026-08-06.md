...
- **Owner:** Product/Release manager and environment tester
- **Status (2026-08-14):** In progress — the live AURIS360 Supabase project passed the complete read-only Core Control gate verification: 4/4 rollout relations, transition trigger installed, RLS enabled on 3/3 protected tables, no unsafe enabled cohort policy, no open rollout-health finding and 118 Core Control records with no missing company relationship. No cohort policy is saved, so all tenants remain in compatibility mode. Supabase Advisor reports no issues. Authenticated Dashboard and all four Core Control modules rendered live data without schema/setup errors; 1440, 1024 and 390 px smoke checks had no page-level horizontal overflow. A session-scoped administrator resilience simulator safely exercises optional dependency failure paths without database writes. Settings now also includes a synthetic, session-only mobile/offline preflight for local storage, photo retention, duplicate protection and company isolation, and the production queue has a single-flight reconnect guard. Automated resilience, mobile/offline and rollback contracts protect the primary registers, PWA/deep-link flows and controlled cohort reversal. Captured live resilience evidence, real-device Incident/BBS offline synchronization and a real Pilot-to-Paused-to-Disabled exercise remain pending; no cohort was enabled and no production record was changed.
