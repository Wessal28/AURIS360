...
- **Owner:** Product/Release manager and environment tester
- **Status (2026-08-13):** In progress — the live AURIS360 Supabase project passed nine rollout schema/RLS checks and the Core Control data baseline found 118 records with no missing company relationship. All ten companies remain in compatibility mode with no saved cohort or health-event rows. The `pending_notifications_security_upgrade.sql` correction was applied and Supabase Advisor reports no issues. Authenticated Dashboard and all four Core Control modules rendered live data without schema/setup errors; 1440, 1024 and 390 px smoke checks had no page-level horizontal overflow. The corrected bounded post-authentication deep-link retry is deployed and contract-protected. Role-matrix validation covers seven roles across all four modules. The shared Core Control workflow contract now blocks stage-skipping and terminal-record reopening while protecting action activity/audit/notification evidence, Incident and Inspection source links, and Risk approval closure/audit/notification evidence. Automated resilience and mobile/offline contracts now protect the dashboard, all four primary registers, the installable PWA shell, role-gated mobile navigation, authenticated deep links and tenant-safe Incident/BBS offline queues. The stale Inspection sync tag was replaced by the supported BBS Observation workflow, and background sync now requests real authenticated queue processing. Live resilience, mobile/offline and rollback evidence remain pending; no cohort was enabled and no production record was changed.
