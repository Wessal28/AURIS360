...
const cssPath = path.join(root, 'auris-settings-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-settings"');
const end = index.indexOf('<div id="page-sitemap"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Settings page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Settings by scripts/extract-settings-styles.cjs. */',
    '/* Non-important declarations preserve branding, security, diagnostics, notification and onboarding runtime states. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-settings-static.css') || !fs.existsSync(cssPath)) throw new Error('Settings stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-settings-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Settings stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Settings styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-settings-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-settings-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-settings-static.css')) {
...
console.log(`Extracted ${extractedCount} Settings style attributes into ${styles.size} reusable rules.`);
