...
const cssPath = path.join(root, 'auris-tools-equipment-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-tools"');
const end = index.indexOf('<div id="page-permit"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Tools & Equipment page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Tools & Equipment by scripts/extract-tools-equipment-styles.cjs. */',
    '/* Non-important declarations preserve register, inspection, assurance and release runtime state. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-tools-equipment-static.css') || !fs.existsSync(cssPath)) throw new Error('Tools & Equipment stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-tools-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Tools & Equipment stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Tools & Equipment styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-tools-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-tools-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-tools-equipment-static.css')) {
...
console.log(`Extracted ${extractedCount} Tools & Equipment style attributes into ${styles.size} reusable rules.`);
