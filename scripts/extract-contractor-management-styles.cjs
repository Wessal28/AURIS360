...
const cssPath = path.join(root, 'auris-contractor-management-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-contractor"');
const end = index.indexOf('<div id="page-esg"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Contractor Management page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Contractor Management by scripts/extract-contractor-management-styles.cjs. */',
    '/* Non-important declarations preserve onboarding, performance and access-control runtime state. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-contractor-management-static.css') || !fs.existsSync(cssPath)) throw new Error('Contractor Management stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-contractor-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Contractor Management stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Contractor Management styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-contractor-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-contractor-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-contractor-management-static.css')) {
...
console.log(`Extracted ${extractedCount} Contractor Management style attributes into ${styles.size} reusable rules.`);
