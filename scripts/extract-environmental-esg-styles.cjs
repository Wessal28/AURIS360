...
const cssPath = path.join(root, 'auris-environmental-esg-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-esg"');
const end = index.indexOf('<div id="page-emergency"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Environmental / ESG page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Environmental / ESG by scripts/extract-environmental-esg-styles.cjs. */',
    '/* Non-important declarations preserve waste, resource, spill and inspection runtime state. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-environmental-esg-static.css') || !fs.existsSync(cssPath)) throw new Error('Environmental / ESG stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-esg-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Environmental / ESG stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Environmental / ESG styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-esg-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-esg-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-environmental-esg-static.css')) {
  nextIndex = nextIndex.replace(
    '<link rel="stylesheet" href="auris-contractor-management-static.css?v=20260815-1">',
...
console.log(`Extracted ${extractedCount} Environmental / ESG style attributes into ${styles.size} reusable rules.`);
