...
const cssPath = path.join(root, 'auris-hse-meetings-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-meetings"');
const end = index.indexOf('<div id="page-training"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded HSE Meetings page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from HSE Meetings by scripts/extract-hse-meetings-styles.cjs. */',
    '/* Non-important declarations preserve meeting, minutes, toolbox, alert and bulletin runtime state. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-hse-meetings-static.css') || !fs.existsSync(cssPath)) throw new Error('HSE Meetings stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-meetings-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The HSE Meetings stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('HSE Meetings styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-meetings-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-meetings-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-hse-meetings-static.css')) {
...
console.log(`Extracted ${extractedCount} HSE Meetings style attributes into ${styles.size} reusable rules.`);
