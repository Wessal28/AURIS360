...
const cssPath = path.join(root, 'auris-work-schedule.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-workschedule"');
const end = index.indexOf('<div id="page-events"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Work Schedule page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Work Schedule by scripts/extract-work-schedule-styles.cjs. */',
    '/* Non-important declarations preserve view, checklist and work-order runtime state. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-work-schedule.css') || !fs.existsSync(cssPath)) throw new Error('Work Schedule stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-work-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Work Schedule stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Work Schedule styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}
...
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  const className = `auris-work-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-work-schedule.css"')) {
  nextIndex = nextIndex.replace(
    '<link rel="stylesheet" href="auris-kpi-legacy.css?v=20260814-1">',
...
console.log(`Extracted ${extractedCount} Work Schedule style attributes into ${styles.size} reusable rules.`);
