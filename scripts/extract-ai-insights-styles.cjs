...
const cssPath = path.join(root, 'auris-ai-insights-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-ai-insights"');
const end = index.indexOf('<div id="page-approvals"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded AI Insights page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from AI Insights by scripts/extract-ai-insights-styles.cjs. */',
    '/* Non-important declarations preserve insight, generation, evidence, recommendation and chat runtime states. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-ai-insights-static.css') || !fs.existsSync(cssPath)) throw new Error('AI Insights stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-ai-insights-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The AI Insights stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('AI Insights styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-ai-insights-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-ai-insights-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-ai-insights-static.css')) {
...
console.log(`Extracted ${extractedCount} AI Insights style attributes into ${styles.size} reusable rules.`);
