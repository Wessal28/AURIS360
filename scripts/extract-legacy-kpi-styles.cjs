...
const cssPath = path.join(root, 'auris-kpi-legacy.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-kpi"');
const end = index.indexOf('<div id="page-workschedule"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded legacy KPI page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from the legacy Objectives & KPIs surface by scripts/extract-legacy-kpi-styles.cjs. */',
    '/* Non-important declarations preserve modal, tab and monthly-entry runtime state. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-kpi-legacy.css') || !fs.existsSync(cssPath)) throw new Error('Legacy KPI stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-kpilegacy-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The legacy KPI stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Legacy KPI styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}
...
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  const className = `auris-kpilegacy-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-kpi-legacy.css"')) {
...
console.log(`Extracted ${extractedCount} legacy KPI style attributes into ${styles.size} reusable rules.`);
