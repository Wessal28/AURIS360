...
const cssPath = path.join(root, 'auris-ppe-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-ppe"');
const end = index.indexOf('<div id="page-noise"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded PPE page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from PPE by scripts/extract-ppe-styles.cjs. */',
    '/* Non-important declarations preserve catalogue, inventory, issue, inspection and replacement runtime state. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-ppe-static.css') || !fs.existsSync(cssPath)) throw new Error('PPE stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-ppe-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The PPE stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('PPE styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-ppe-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-ppe-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-ppe-static.css')) {
...
console.log(`Extracted ${extractedCount} PPE style attributes into ${styles.size} reusable rules.`);
