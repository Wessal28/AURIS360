...
const cssPath = path.join(root, 'auris-audits-inspections-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-inspection"');
const end = index.indexOf('<div id="page-risk"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Audits & Inspections page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Audits & Inspections by scripts/extract-audits-inspections-styles.cjs. */',
    '/* Non-important declarations preserve audit, pre-start, finding and evidence runtime state. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-audits-inspections-static.css') || !fs.existsSync(cssPath)) throw new Error('Audits & Inspections stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-audit-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Audits & Inspections stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Audits & Inspections styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-audit-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-audit-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-audits-inspections-static.css')) {
...
console.log(`Extracted ${extractedCount} Audits & Inspections style attributes into ${styles.size} reusable rules.`);
