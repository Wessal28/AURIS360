...
const cssPath = path.join(root, 'auris-document-control-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-documents"');
const end = index.indexOf('<div id="page-people"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Document Control page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Document Control by scripts/extract-document-control-styles.cjs. */',
    '/* Non-important declarations preserve register, revision, approval, distribution, acknowledgement, archive and viewer runtime states. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-document-control-static.css') || !fs.existsSync(cssPath)) throw new Error('Document Control stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-documents-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Document Control stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Document Control styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-documents-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-documents-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-document-control-static.css')) {
  nextIndex = nextIndex.replace(
    '<link rel="stylesheet" href="auris-swms-static.css?v=20260815-1">',
    '<link rel="stylesheet" href="auris-swms-static.css?v=20260815-1">\n<link rel="stylesheet" href="auris-document-control-static.css?v=20260815-1">'
  );
}
fs.writeFileSync(indexPath, nextIndex);
fs.writeFileSync(cssPath, renderCss(migrated, styles));
console.log(`Extracted ${extractedCount} Document Control style attributes into ${styles.size} reusable rules.`);
