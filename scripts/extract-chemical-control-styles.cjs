...
const cssPath = path.join(root, 'auris-chemical-control-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-chemical"');
const end = index.indexOf('<div id="page-legal"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Chemical Control page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Chemical Control by scripts/extract-chemical-control-styles.cjs. */',
    '/* Non-important declarations preserve SDS, inventory, risk and review runtime state. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-chemical-control-static.css') || !fs.existsSync(cssPath)) throw new Error('Chemical Control stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-chemical-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Chemical Control stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Chemical Control styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-chemical-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-chemical-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-chemical-control-static.css')) {
  nextIndex = nextIndex.replace(
    '<link rel="stylesheet" href="auris-risk-assessment-static.css?v=20260814-1">',
...
console.log(`Extracted ${extractedCount} Chemical Control style attributes into ${styles.size} reusable rules.`);
