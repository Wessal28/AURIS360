...
const cssPath = path.join(root, 'auris-administration-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-admin"');
const end = index.indexOf('<div id="page-integrations"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Companies / Administration page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Companies / Administration by scripts/extract-administration-styles.cjs. */',
    '/* Non-important declarations preserve company, site, hierarchy, oversight, access and module runtime states. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-administration-static.css') || !fs.existsSync(cssPath)) throw new Error('Companies / Administration stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-administration-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Companies / Administration stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Companies / Administration styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-administration-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-administration-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-administration-static.css')) {
  nextIndex = nextIndex.replace(
    '<link rel="stylesheet" href="auris-users-roles-static.css?v=20260815-1">',
    '<link rel="stylesheet" href="auris-users-roles-static.css?v=20260815-1">\n<link rel="stylesheet" href="auris-administration-static.css?v=20260815-1">'
  );
}
fs.writeFileSync(indexPath, nextIndex);
fs.writeFileSync(cssPath, renderCss(migrated, styles));
console.log(`Extracted ${extractedCount} Companies / Administration style attributes into ${styles.size} reusable rules.`);
