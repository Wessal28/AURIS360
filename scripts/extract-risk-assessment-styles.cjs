...
const cssPath = path.join(root, 'auris-risk-assessment-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-risk"');
const end = index.indexOf('<div id="page-investigation"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Risk Assessment page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Risk Assessment by scripts/extract-risk-assessment-styles.cjs. */',
    '/* Non-important declarations preserve assessment, hazard, approval and library runtime state. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-risk-assessment-static.css') || !fs.existsSync(cssPath)) throw new Error('Risk Assessment stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-risk-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Risk Assessment stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Risk Assessment styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-risk-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-risk-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-risk-assessment-static.css')) {
  nextIndex = nextIndex.replace(
    '<link rel="stylesheet" href="auris-audits-inspections-static.css?v=20260814-1">',
...
console.log(`Extracted ${extractedCount} Risk Assessment style attributes into ${styles.size} reusable rules.`);
