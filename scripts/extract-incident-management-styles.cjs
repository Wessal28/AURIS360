...
const cssPath = path.join(root, 'auris-incident-management-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-events"');
const end = index.indexOf('<div id="page-observation"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Incident Management page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Incident Management by scripts/extract-incident-management-styles.cjs. */',
    '/* Non-important declarations preserve report, investigation, evidence and action runtime state. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-incident-management-static.css') || !fs.existsSync(cssPath)) throw new Error('Incident Management stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-incident-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Incident Management stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Incident Management styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}
...
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  const className = `auris-incident-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-incident-management-static.css')) {
  nextIndex = nextIndex.replace(
    '<link rel="stylesheet" href="auris-work-schedule.css?v=20260814-1">',
...
console.log(`Extracted ${extractedCount} Incident Management style attributes into ${styles.size} reusable rules.`);
