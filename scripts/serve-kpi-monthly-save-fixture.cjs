...
const panel=html.slice(panelStart,panelEnd+'</div></div></div>'.length);
...
const assets={
  '/':['text/html',read('tests/fixtures/kpi-monthly-save.html').replace('<!-- MONTHLY_PANEL -->',panel)],
  '/fixture-core.js':['text/javascript',core.slice(core.indexOf('async function kpiLoadAll('),core.indexOf('function kpiUpdateMetrics()'))+core.slice(core.indexOf('function closeKpiModal(id)'),core.indexOf('async function kpiDeleteObjective'))+core.slice(core.indexOf('function kpiCalcYTD(indicatorId'),core.indexOf('async function kpiClearEntry()'))],
  '/fixture-module.js':['text/javascript',upgrade.replace(boot,"function fixtureBoot(){kpiXEnhanceEntryModal();kpiXInstallHooks();window.kpiRenderOverview=function(){};window.kpiRenderMonthly=function(){};window.kpiUpdateMetrics=function(){};kpiXRenderAll=function(){};document.getElementById('kpi-entry-modal').setAttribute('role','dialog');document.getElementById('kpi-entry-modal').setAttribute('aria-label','Monthly KPI entry');}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',fixtureBoot);else fixtureBoot();")]
};
