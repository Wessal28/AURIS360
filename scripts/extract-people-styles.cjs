...
const cssPath = path.join(root, 'auris-people-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-people"');
const end = index.indexOf('<div id="page-users"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded People page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from People by scripts/extract-people-styles.cjs. */',
    '/* Non-important declarations preserve employee, contractor, HSE and register runtime states. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-people-static.css') || !fs.existsSync(cssPath)) throw new Error('People stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-people-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The People stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('People styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-people-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-people-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-people-static.css')) {
...
console.log(`Extracted ${extractedCount} People style attributes into ${styles.size} reusable rules.`);
