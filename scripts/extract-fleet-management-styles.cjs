...
const cssPath = path.join(root, 'auris-fleet-management-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-fleet"');
const end = index.indexOf('<div id="page-atex"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Fleet Management page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Fleet Management by scripts/extract-fleet-management-styles.cjs. */',
    '/* Non-important declarations preserve fleet metrics, filters and access-controlled actions. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-fleet-management-static.css') || !fs.existsSync(cssPath)) throw new Error('Fleet Management stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-fleet-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Fleet Management stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Fleet Management styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-fleet-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-fleet-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-fleet-management-static.css')) {
  nextIndex = nextIndex.replace(
    '<link rel="stylesheet" href="auris-legal-compliance-static.css?v=20260814-1">',
    '<link rel="stylesheet" href="auris-legal-compliance-static.css?v=20260814-1">\n<link rel="stylesheet" href="auris-fleet-management-static.css?v=20260814-1">'
  );
}
fs.writeFileSync(indexPath, nextIndex);
fs.writeFileSync(cssPath, renderCss(migrated, styles));
console.log(`Extracted ${extractedCount} Fleet Management style attributes into ${styles.size} reusable rules.`);
