...
const cssPath = path.join(root, 'auris-occupational-noise-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-noise"');
const end = index.indexOf('<div id="page-meetings"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Occupational Noise Management page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Occupational Noise Management by scripts/extract-occupational-noise-styles.cjs. */',
    '/* Non-important declarations preserve survey, map, HPE and editor runtime state. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-occupational-noise-static.css') || !fs.existsSync(cssPath)) throw new Error('Occupational Noise Management stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-noise-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Occupational Noise Management stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Occupational Noise Management styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-noise-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-noise-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-occupational-noise-static.css')) {
  nextIndex = nextIndex.replace(
    '<link rel="stylesheet" href="auris-ppe-static.css?v=20260815-1">',
    '<link rel="stylesheet" href="auris-ppe-static.css?v=20260815-1">\n<link rel="stylesheet" href="auris-occupational-noise-static.css?v=20260815-1">'
  );
}
fs.writeFileSync(indexPath, nextIndex);
fs.writeFileSync(cssPath, renderCss(migrated, styles));
console.log(`Extracted ${extractedCount} Occupational Noise Management style attributes into ${styles.size} reusable rules.`);
