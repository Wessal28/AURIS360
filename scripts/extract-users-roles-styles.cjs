...
const cssPath = path.join(root, 'auris-users-roles-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-users"');
const end = index.indexOf('<div id="page-admin"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Users & Roles page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Users & Roles by scripts/extract-users-roles-styles.cjs. */',
    '/* Non-important declarations preserve invitation, account, role, company-access and credential runtime states. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-users-roles-static.css') || !fs.existsSync(cssPath)) throw new Error('Users & Roles stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-users-roles-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Users & Roles stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Users & Roles styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-users-roles-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-users-roles-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-users-roles-static.css')) {
  nextIndex = nextIndex.replace(
    '<link rel="stylesheet" href="auris-people-static.css?v=20260815-1">',
...
console.log(`Extracted ${extractedCount} Users & Roles style attributes into ${styles.size} reusable rules.`);
