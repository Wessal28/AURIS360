...
async function main() {
  ...
  const assets = [
    ...
    ['auris-integration-engine.js', ["version:'1.3.0'", 'connection:connection', 'event:event', 'delivery:delivery', 'exportCsv:exportCsv', 'parseCsv:parseCsv', 'importPlan:importPlan', 'stagedImport:stagedImport', 'stagedReconciliation:stagedReconciliation', 'mappingProfile:mappingProfile', 'mappedImport:mappedImport', 'stagedMappedImport:stagedMappedImport']],
    ['auris-integration-centre.js', ["version:'1.3.0'", 'mount:mount', 'load:load', 'canManage:canManage', 'Reusable CSV mapping profiles', 'Validate with ', 'stage_mapped_action_batch', 'Safe rollback']],
