...
const cssPath = path.join(root, 'auris-atex-areas-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-atex"');
const end = index.indexOf('<div id="page-tools"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded ATEX Areas page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from ATEX Areas by scripts/extract-atex-areas-styles.cjs. */',
    '/* Non-important declarations preserve zone, control, inspection and status runtime state. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-atex-areas-static.css') || !fs.existsSync(cssPath)) throw new Error('ATEX Areas stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-atex-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The ATEX Areas stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('ATEX Areas styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-atex-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-atex-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-atex-areas-static.css')) {
  nextIndex = nextIndex.replace(
    '<link rel="stylesheet" href="auris-fleet-management-static.css?v=20260814-1">',
    '<link rel="stylesheet" href="auris-fleet-management-static.css?v=20260814-1">\n<link rel="stylesheet" href="auris-atex-areas-static.css?v=20260814-1">'
  );
}
fs.writeFileSync(indexPath, nextIndex);
fs.writeFileSync(cssPath, renderCss(migrated, styles));
console.log(`Extracted ${extractedCount} ATEX Areas style attributes into ${styles.size} reusable rules.`);
