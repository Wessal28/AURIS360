...
const cssPath = path.join(root, 'auris-master-action-plan-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-actions"');
const end = index.indexOf('<div id="page-sop"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Master Action Plan page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Master Action Plan by scripts/extract-master-action-plan-styles.cjs. */',
    '/* Non-important declarations preserve assignment, escalation, progress, verification and closure runtime state. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-master-action-plan-static.css') || !fs.existsSync(cssPath)) throw new Error('Master Action Plan stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-map-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Master Action Plan stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Master Action Plan styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-map-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-map-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-master-action-plan-static.css')) {
  nextIndex = nextIndex.replace(
    '<link rel="stylesheet" href="auris-management-of-change-static.css?v=20260815-1">',
    '<link rel="stylesheet" href="auris-management-of-change-static.css?v=20260815-1">\n<link rel="stylesheet" href="auris-master-action-plan-static.css?v=20260815-1">'
  );
}
fs.writeFileSync(indexPath, nextIndex);
fs.writeFileSync(cssPath, renderCss(migrated, styles));
console.log(`Extracted ${extractedCount} Master Action Plan style attributes into ${styles.size} reusable rules.`);
