...
const cssPath = path.join(root, 'auris-site-map-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-sitemap"');
const end = index.indexOf('<div id="page-fire"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Site Map page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Site Map by scripts/extract-site-map-styles.cjs. */',
    '/* Non-important declarations preserve site, plan, overlay, marker, canvas and detail runtime states. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-site-map-static.css') || !fs.existsSync(cssPath)) throw new Error('Site Map stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-site-map-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Site Map stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Site Map styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-site-map-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-site-map-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-site-map-static.css')) {
  nextIndex = nextIndex.replace(
    '<link rel="stylesheet" href="auris-settings-static.css?v=20260815-1">',
...
console.log(`Extracted ${extractedCount} Site Map style attributes into ${styles.size} reusable rules.`);
