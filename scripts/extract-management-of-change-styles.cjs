...
const cssPath = path.join(root, 'auris-management-of-change-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-moc"');
const end = index.indexOf('<div id="page-actions"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Management of Change page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Management of Change by scripts/extract-management-of-change-styles.cjs. */',
    '/* Non-important declarations preserve register, review, approval, implementation and closure runtime state. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-management-of-change-static.css') || !fs.existsSync(cssPath)) throw new Error('Management of Change stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-moc-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Management of Change stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Management of Change styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-moc-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-moc-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-management-of-change-static.css')) {
  nextIndex = nextIndex.replace(
    '<link rel="stylesheet" href="auris-training-competency-static.css?v=20260815-1">',
    '<link rel="stylesheet" href="auris-training-competency-static.css?v=20260815-1">\n<link rel="stylesheet" href="auris-management-of-change-static.css?v=20260815-1">'
  );
}
fs.writeFileSync(indexPath, nextIndex);
fs.writeFileSync(cssPath, renderCss(migrated, styles));
console.log(`Extracted ${extractedCount} Management of Change style attributes into ${styles.size} reusable rules.`);
