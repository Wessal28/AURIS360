...
const cssPath = path.join(root, 'auris-emergency-management-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-emergency"');
const end = index.indexOf('<div id="page-ohealth"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Emergency Management page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Emergency Management by scripts/extract-emergency-management-styles.cjs. */',
    '/* Non-important declarations preserve emergency plans, drills, activations and equipment runtime state. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-emergency-management-static.css') || !fs.existsSync(cssPath)) throw new Error('Emergency Management stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-emergency-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Emergency Management stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Emergency Management styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-emergency-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-emergency-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-emergency-management-static.css')) {
  nextIndex = nextIndex.replace(
    '<link rel="stylesheet" href="auris-environmental-esg-static.css?v=20260815-1">',
    '<link rel="stylesheet" href="auris-environmental-esg-static.css?v=20260815-1">\n<link rel="stylesheet" href="auris-emergency-management-static.css?v=20260815-1">'
  );
}
fs.writeFileSync(indexPath, nextIndex);
fs.writeFileSync(cssPath, renderCss(migrated, styles));
console.log(`Extracted ${extractedCount} Emergency Management style attributes into ${styles.size} reusable rules.`);
