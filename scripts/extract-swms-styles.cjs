...
const cssPath = path.join(root, 'auris-swms-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-swms"');
const end = index.indexOf('<div id="page-documents"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded SWMS / Method Statements page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from SWMS / Method Statements by scripts/extract-swms-styles.cjs. */',
    '/* Non-important declarations preserve register, briefing, verification, change and editor runtime state. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-swms-static.css') || !fs.existsSync(cssPath)) throw new Error('SWMS stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-swms-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The SWMS stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('SWMS styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-swms-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-swms-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-swms-static.css')) {
...
console.log(`Extracted ${extractedCount} SWMS style attributes into ${styles.size} reusable rules.`);
