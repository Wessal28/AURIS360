...
const cssPath = path.join(root, 'auris-legal-compliance-static.css');
const index = fs.readFileSync(indexPath, 'utf8');
const start = index.indexOf('<div id="page-legal"');
const end = index.indexOf('<div id="page-fleet"', start);
if (start < 0 || end < 0) throw new Error('Could not locate the bounded Legal Compliance page.');
...
function renderCss(section, styles) {
  ...
  return [
    '/* Generated from Legal Compliance by scripts/extract-legal-compliance-styles.cjs. */',
    '/* Non-important declarations preserve register, assessment, remediation and calendar runtime state. */',
    '/* Governed legal checkboxes retain required 18px important dimensions over the global 16px checkbox rule. */',
...
if (extractedCount === 0) {
  if (!index.includes('href="auris-legal-compliance-static.css') || !fs.existsSync(cssPath)) throw new Error('Legal Compliance stylesheet is not installed.');
  const installedStyles = new Map();
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-legal-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) installedStyles.set(match[1], match[2]);
  if (!installedStyles.size) throw new Error('The Legal Compliance stylesheet has no generated rules.');
  fs.writeFileSync(cssPath, renderCss(source, installedStyles));
  console.log('Legal Compliance styles are already externalized; specificity rules refreshed.');
  process.exit(0);
}

const styles = new Map();
if (fs.existsSync(cssPath)) {
  for (const match of fs.readFileSync(cssPath, 'utf8').matchAll(/^\.(auris-legal-s-[a-f0-9]{10})\{([^}]*)\}$/gm)) styles.set(match[1], match[2]);
}
const migrated = source.replace(/<[A-Za-z][^<>]*>/g, (tag) => {
  ...
  while ((styleMatch = result.match(/\sstyle=("([^"]*)"|'([^']*)')/i))) {
    ...
    const className = `auris-legal-s-${crypto.createHash('sha256').update(declaration).digest('hex').slice(0, 10)}`;
...
if (!nextIndex.includes('href="auris-legal-compliance-static.css')) {
  nextIndex = nextIndex.replace(
    '<link rel="stylesheet" href="auris-chemical-control-static.css?v=20260814-1">',
    '<link rel="stylesheet" href="auris-chemical-control-static.css?v=20260814-1">\n<link rel="stylesheet" href="auris-legal-compliance-static.css?v=20260814-1">'
  );
}
fs.writeFileSync(indexPath, nextIndex);
fs.writeFileSync(cssPath, renderCss(migrated, styles));
console.log(`Extracted ${extractedCount} Legal Compliance style attributes into ${styles.size} reusable rules.`);
