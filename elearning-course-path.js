(function(){
  ...
  function ensureBuilder(){
    ...
    var group=legacy.closest('.form3group');if(group){
      ...
      group.querySelector('.muted')?.insertAdjacentHTML('beforebegin','<div class="muted" style="font-size:11px;margin-top:6px">Existing single-video courses continue to use this link. New courses should use the structured video path below.</div>');
    }
    var card=document.createElement('div');card.id='elc-course-path';card.className='elc-path-card';
    card.innerHTML='<div class="elc-path-head"><div><div class="elc-path-title">Course videos and final quiz</div><div class="elc-path-sub">Add short videos in viewing order. Learners complete them before the final quiz is unlocked.</div></div><button type="button" class="btn btn-sm" onclick="elcPathAddLesson()"><i class="ti ti-plus"></i>Add video</button></div><div id="elc-path-lessons"></div><div class="elc-path-head" style="margin-top:16px"><div><div class="elc-path-title">Final quiz</div><div class="elc-path-sub">The pass mark uses the course passing score above.</div></div><button type="button" class="btn btn-sm" onclick="elcPathAddQuestion()"><i class="ti ti-plus"></i>Add question</button></div><label style="display:flex;align-items:center;gap:8px;font-size:12px;font-weight:700"><input id="elc-quiz-enabled" type="checkbox" checked onchange="elcPathToggleQuiz(this.checked)"> Require final quiz</label><div id="elc-path-questions"></div>';
  ...
  window.eleOpenVideoPlayer=function(opts){
    ...
    modal.innerHTML='<div class="ele-video-shell ele-course-shell" role="dialog" aria-modal="true"><div class="ele-video-head"><div><div style="font-size:17px;font-weight:900">'+escH(course.title||'E-learning course')+'</div><div style="font-size:12px;opacity:.85">'+escH(preview?'Course preview':'Structured learning path')+'</div></div><button class="icon-btn" style="background:rgba(255,255,255,.18);color:#fff" onclick="eleCloseVideoPlayer()"><i class="ti ti-x"></i></button></div><div class="ele-video-body"><div class="ele-course-layout"><div id="ele-course-stage" class="ele-course-stage"></div><aside id="ele-course-outline" class="ele-course-outline"></aside></div></div></div>';
    ...
    function renderVideo(){var l=lessons[index],stage=document.getElementById('ele-course-stage');stage.innerHTML='<h3 style="margin:0 0 10px">'+escH((index+1)+'. '+l.title)+'</h3><video id="ele-watch-video" class="ele-watch-player" controls controlsList="nodownload noplaybackrate noremoteplayback" disablePictureInPicture playsinline preload="metadata" oncontextmenu="return false" src="'+escH(l.video_url)+'"></video><div class="ele-watch-note">Viewing only. Download controls and the context menu are disabled. Complete this video to unlock the next item.</div><div class="ele-course-nav"><button class="btn" '+(index===0?'disabled':'')+' onclick="window._eleCourseGo('+(index-1)+')"><i class="ti ti-chevron-left"></i>Previous</button><button id="ele-path-next" class="btn btn-primary" '+(!preview&&!progress[l.id]?.completed?'disabled':'')+' onclick="window._eleCourseGo('+(index+1)+')">Next<i class="ti ti-chevron-right"></i></button></div>';var video=document.getElementById('ele-watch-video');video.addEventListener('ended',async function(){await saveLesson(l,video);var b=document.getElementById('ele-path-next');if(b)b.disabled=false;toast(index===lessons.length-1&&quiz.enabled?'Videos complete. Final quiz unlocked.':'Video complete. Next item unlocked.');});}
    function renderQuiz(){var stage=document.getElementById('ele-course-stage');if(!allVideosDone()){index=Math.max(0,lessons.length-1);renderVideo();return;}stage.innerHTML='<div class="ele-quiz-panel"><h3 style="margin-top:0">Final quiz</h3><p class="muted">Pass mark: '+quiz.passing_score+'%</p><form id="ele-final-quiz">'+quiz.questions.map(function(q,i){return '<div class="ele-quiz-question"><strong>'+(i+1)+'. '+escH(q.prompt)+'</strong>'+q.options.map(function(o,j){return '<label class="ele-quiz-option"><input type="radio" name="q'+i+'" value="'+j+'"> <span>'+escH(o)+'</span></label>';}).join('')+'</div>';}).join('')+'<button type="submit" class="btn btn-primary" style="margin-top:14px"><i class="ti ti-check"></i>Submit quiz</button></form></div>';document.getElementById('ele-final-quiz').addEventListener('submit',submitQuiz);}
