...
function mount(host,rows,options){
  ...
  return root.AurisViewEngine.mount(host,projected,{moduleKey:'atex-areas',label:'ATEX Areas',definition:definition(),context:function(){return assertSession(current);},filters:filters(options.filters),actions:[{key:'open',label:'View ATEX area',href:typeof options.recordHref==='function'?function(row){return options.recordHref(row.id,'view');}:undefined},{key:'edit',label:'Edit ATEX area',href:typeof options.recordHref==='function'?function(row){return options.recordHref(row.id,'edit');}:undefined,when:function(){return options.canEdit===true;}}],onApplyFilters:function(value){assertSession(current);if(typeof options.onApplyFilters==='function')options.onApplyFilters(filters(value));},onAction:async function(action,row){
...
root.AurisAtexListWorkspace=Object.freeze({version:'1.0.0',session:session,assertSession:assertSession,mount:mount,project:project,definition:definition,filters:filters,dateState:dateState});
