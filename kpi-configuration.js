...
function merge(base,extra){Object.keys(extra||{}).forEach(function(k){if(extra[k]&&typeof extra[k]==='object'&&!Array.isArray(extra[k]))base[k]=merge(base[k]&&typeof base[k]==='object'&&!Array.isArray(base[k])?base[k]:{},extra[k]);else base[k]=extra[k];});return base;}
...
function input(path,label,type,help,options,forceDisabled){var v=val(path),disabled=editable()&&!forceDisabled?'':' disabled',id='kpi-cfg-'+path.replace(/\./g,'-');if(type==='toggle')return '<label class="kpi-cfg-toggle"><span><strong>'+esc(label)+'</strong><small>'+esc(help||'')+'</small></span><input id="'+id+'" type="checkbox" data-cfg="'+path+'" '+(v?'checked':'')+disabled+' data-auris-module-onchange="b0001"><i></i></label>';
  ...
  else field+='<input id="'+id+'" data-cfg="'+path+'"'+(type==='number'?' min="0" max="100" step="any" required':'')+' type="'+(type||'text')+'" value="'+esc(v==null?'':v)+'"'+disabled+' data-auris-module-oninput="b0002">';
...
function sectionTargets(){var t=cfg().targets;return panel('Targets & Status Rules',grid([
  input('targets.on_track_percent','KPI On Track threshold (%)','number','Applies when worst-indicator status is disabled. Indicators still use their target operator to determine On Track.'),input('targets.at_risk_percent','At Risk threshold (%)','number','Warning band for indicators that miss their target, and for score-based KPI status.'),input('targets.zero_tolerance_override','Zero-tolerance override','toggle','Forces non-zero results Off Track. The zero-tolerance operator already scores them as zero, even with this override off.'),input('targets.critical_override','Use worst indicator status for KPI','toggle','Use the highest-priority indicator status. This applies to all indicators, not a separate critical flag.'),input('cycles.current_period_excluded','Exclude open current period','toggle','The current month remains In Progress and does not reduce dashboard achievement.')
]))+panel('Score-based KPI Threshold Preview',(thresholdErrors(cfg()).length?'<p class="kpi-cfg-help">Enter both thresholds between 0 and 100, with At Risk below On Track, to preview the bands.</p>':'<div class="kpi-cfg-threshold"><span class="on">On Track <b>≥ '+esc(t.on_track_percent)+'%</b></span><span class="risk">At Risk <b>≥ '+esc(t.at_risk_percent)+'% and &lt; '+esc(t.on_track_percent)+'%</b></span><span class="off">Off Track <b>&lt; '+esc(t.at_risk_percent)+'%</b></span><span class="missing">Data Missing <b>required value absent</b></span></div>'),'These bands classify KPI scores when worst-indicator status is disabled; individual indicators retain their target-operator checks.');}
function sectionCalculations(){return panel('Calculation Defaults',grid([
  input('calculations.aggregation','Indicator aggregation within each KPI','select','',[['average','Average indicator achievement'],['worst','Lowest indicator achievement']])
]),'Objective totals remain the average of reported KPI scores. This setting only combines indicator achievement inside each KPI; YTD remains configured on each indicator.');}
function sectionSources(){return panel('Data Source & Reconciliation',grid([
  input('sources.default_source','Default source for new KPIs','select','Sets the editor default; Evidence / document does not connect an external service. Module metrics must be configured on each indicator.',[['manual','Manual entry'],['module','AURIS360 module'],['integration','Evidence / document']]),input('sources.refresh_frequency','Automatic refresh frequency','select','Checked when KPI data loads, not by a background scheduler. Historical years and manual overrides are excluded.',[['real_time','On each KPI load'],['hourly','Hourly'],['daily','Daily'],['monthly','Monthly']]),input('sources.allow_manual_override','Allow manual override','toggle','Authorised users may replace an automatically calculated result; the override reason is audited.')
...
]),'The published route assigns monthly submission, verification and approval, and supplies defaults for new KPI definitions. Existing submissions retain their saved route. Each selected person must have one active company account; without self-approval, all three people must be different.');}
...
function thresholdErrors(c){
  var values=[c.targets.at_risk_percent,c.targets.on_track_percent];
  if(values.some(function(value){return !((typeof value==='number'||typeof value==='string'&&value.trim()!=='')&&Number.isFinite(Number(value))&&Number(value)>=0&&Number(value)<=100);}))return ['Enter both status thresholds as numbers between 0 and 100.'];
  return Number(values[0])>=Number(values[1])?['At Risk threshold must be lower than On Track.']:[];
}
function validate(){var c=cfg(),errors=thresholdErrors(c),warnings=[];if(['average','worst'].indexOf(c.calculations.aggregation)<0)errors.push('Select a supported indicator aggregation method.');if(!c.workflow.self_approval&&String(c.workflow.stage1).toLowerCase()===String(c.workflow.stage2).toLowerCase())errors.push('The submitter and verifier stages cannot be identical.');return {valid:errors.length===0,errors:errors,warnings:warnings,validated_at:new Date().toISOString()};}
function impact(){
  try{if(typeof window.kpiXPreviewConfiguration==='function')return window.kpiXPreviewConfiguration(clone(cfg()));}catch(error){/* A preview failure must not change live settings or invent a zero impact. */}
  return {available:false,generated_at:new Date().toISOString()};
}
function impactFeedback(current){
  var p=current.impact_summary||{};
  if(current.status!=='validated'||!current.validation||!current.validation.valid)return '';
  if(p.available!==true)return '<p class="kpi-cfg-help">Impact preview unavailable. Validate again with KPI reporting data loaded. Validation does not certify reporting outcomes.</p>';
  function month(value){return value?['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][value-1]:'No compiled month';}
  return '<p class="kpi-cfg-help">Snapshot at validation time for '+esc(p.reporting_year)+'. Published and draft settings are compared using loaded results only; this is not a full-history or approval simulation.</p>'+
    '<div class="kpi-cfg-kv"><span>Loaded KPIs compared</span><strong>'+esc(p.kpis_evaluated)+'</strong></div>'+
    '<div class="kpi-cfg-kv"><span>KPI status changes</span><strong>'+esc(p.status_changes)+'</strong></div>'+
    '<div class="kpi-cfg-kv"><span>KPI score changes</span><strong>'+esc(p.score_changes)+'</strong></div>'+
    '<div class="kpi-cfg-kv"><span>Published cut-off</span><strong>'+esc(month(p.published_month))+'</strong></div>'+
    '<div class="kpi-cfg-kv"><span>Draft cut-off</span><strong>'+esc(month(p.draft_month))+'</strong></div>';
}
...
async function validateAndSave(){
  ...
  try{
    ...
    if(current(context)&&revision===state.revision&&!state.notice)notify('Configuration validated. '+(imp.available===true?'Preview: '+imp.status_changes+' status and '+imp.score_changes+' score change(s) among '+imp.kpis_evaluated+' loaded KPIs.':'Impact preview unavailable; validation does not certify reporting outcomes.'));
...
async function publish(){
  ...
  var checks=validate();if(!checks.valid){state.draft.status='draft';state.draft.validation=checks;state.draft.impact_summary={};notify('Validation failed: '+checks.errors.join(' '),false);return false;}
